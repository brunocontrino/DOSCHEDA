...
#'
setGeneric(name="replicatePlot",
           def=function(x, conc, repIndex1, repIndex2, ...)
           {
...
setMethod(f= "replicatePlot",
          signature="ChemoProtSet",
          definition=function(x, conc, repIndex1, repIndex2, ...)
          {
            if(x@parameters$reps > 1){
              ...
              val = max(c(max(data.merged[, index$index1], na.rm = TRUE), max(data.merged[,index$index2],na.rm = TRUE)))


              plot(x=data.merged[,index$index1],y=data.merged[,index$index2], xlim=c(0,val + 0.2), col="green" , ylim=c(0,val+0.2),
                   cex.axis=1.2, main=c(paste0("LogFC-LogFC Plot: Concentration ",conc)),
                   xlab=paste("rep",index$rep1),  ylab=paste("rep",index$rep2))
              text(data.merged[,index$index1], data.merged[,index$index2], labels=data.merged$GeneID, cex= 1,pos=4)
