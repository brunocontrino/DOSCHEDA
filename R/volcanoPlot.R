...
setGeneric(name="volcanoPlot",
           def=function(x, coefficient = 'slope', avExprs = 0.2 , pVal = 0.05, ...)
           {
...
setMethod(f= "volcanoPlot",
          signature="ChemoProtSet",
           definition=function(x, coefficient = 'slope', avExprs = 0.2 , pVal = 0.05, ...)
          {
            res<- x@finalData
            ...
            if(coefficient == 'slope'){
              ...
              with(res, plot(res$AveExpr, -log10(res$P.Value_slope), pch=20, main="Volcano plot (slope pval )",
                             ...
                             xlim=c(-abs(max(res$AveExpr)+1),abs(max(res$AveExpr)+1))))
              ...
              with(res, plot(res$AveExpr, -log10(res$P.Value_intercept), pch=20, main="Volcano plot (Intercept pval )",xlab=c("Log2_AvgFC"),ylab=c("-Log10(Pval)"), xlim=c(-abs(max(res$AveExpr)+1),abs(max(res$AveExpr)+1))))
              ...
              with(res, plot(res$AveExpr, -log10(res$P.Value_quadratic), pch=20, main="Volcano plot (Quadratic pval )",xlab=c("Log2_AvgFC"),ylab=c("-Log10(Pval)"), xlim=c(-abs(max(res$AveExpr)+1),abs(max(res$AveExpr)+1))))
              ...
              # Label points with the textxy function from the calibrate plot
              s=subset(res, P.Value_quadratic < pVal & abs(res$AveExpr) > avExprs)
              with(s, textxy( s$AveExpr, -log10(s$P.Value_quadratic),  labs=s$GeneID, cex=.9)
              )
            } else {
            ...
            legend("bottomleft", title="Legend",cex = 0.7,
                   c("Not significant",
                     paste("P.Value",pVal, sep = " "), #red
                     paste("AvgFC >", avExprs, sep=""), #orange,
                     paste("P.Value", pVal,"& AvgFC >", avExprs, sep="") #green
                   ),
